...
export default function PlanetaryPositionsView({ currentUser }: PlanetaryPositionsViewProps) {
  ...
  const renderZodiacTransitWheel = () => {
    ...
    return (
      <svg viewBox="0 0 280 280" className="w-full max-w-[280px] aspect-square text-amber-600 font-sans mx-auto border border-gray-200 bg-white rounded-full p-2 p-1.5 shadow-none">
        ...
        {[...Array(12)].map((_, idx) => {
          ...
          return (
            <g key={idx} className="stroke-orange-500/20 fill-gray-500 text-[8px] font-bold">
              ...
              <text x={xText} y={yText} textAnchor="middle" className="fill-orange-500/65 font-sans font-bold select-none">{signsAbbrevArr[idx]}</text>
            </g>
          );
        })}

        {/* Draw inner degree ticks rings */}
        <circle cx={center} cy={center} r={radius} fill="none" stroke="#eed37f" strokeWidth="1" strokeOpacity="0.4" />
        <circle cx={center} cy={center} r={radius - 20} fill="none" stroke="#eed37f" strokeWidth="0.6" strokeOpacity="0.2" />
        <circle cx={center} cy={center} r={radius - 50} fill="none" stroke="#eed37f" strokeWidth="0.5" strokeOpacity="0.15" />
        ...
        <circle cx={center} cy={center} r="18" className="fill-[#080403] stroke-orange-500/30 font-sans" />
  ...
  return (
    <div className="space-y-12 pb-16 text-left selection:bg-orange-600/30 selection:text-gray-900" id="planetary-positions-page">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-white via-gray-50 to-white border-b border-gray-200 py-16 px-4 sm:px-6 text-center rounded-lg">
        ...
        <div className="max-w-4xl mx-auto relative z-10 space-y-4">
          <span className="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-white shadow-sm border border-gray-100 border border-gray-200 rounded-full text-[10px] font-sans font-bold text-amber-600 tracking-wider uppercase mb-2">
            Astrological Planetary Positions Terminal
          </span>
          <h1 className="text-amber-600xl sm:text-4xl lg:text-5xl font-sans font-bold text-gray-900 uppercase tracking-tight leading-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center bg-purple-50/30 border border-gray-200 p-6 rounded-lg">
        ...
        <div className="space-y-1">
          <span className="text-[9px] uppercase font-mono tracking-widest text-gray-900 block">OBSERVATIONAL CENTER</span>
          <h4 className="text-sm font-sans font-bold text-gray-900">Sidereal Anchor:</h4>
          <p className="text-xs text-gray-500 font-sans">Coordinates calculate transits aligned to physical zenith angles of chosen region.</p>
        </div>

        <div>
          <label className="text-[10px] text-gray-500 font-sans block mb-1">Set Observation Location</label>
          <div className="relative text-xs">
            ...
            />
            {searchingCity && <RefreshCw className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-amber-600 animate-spin" />}
          </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg mt-1 max-h-44 overflow-y-auto z-40 relative shadow-none py-1 text-left text-[11px] block">
              {suggestions.map((item, idx) => (
                <button
                  ...
                  className="w-full px-3 py-2 hover:bg-purple-50 text-gray-900 hover:text-amber-600 text-left transition-all border-b border-purple-100 block"
                >
                  {item.display_name}
                </button>
              ))}
        ...
        <div className="grid grid-cols-3 gap-2 font-mono text-[10px] text-center">
          <div className="bg-gray-50 border border-gray-200 p-2.5 rounded-lg">
            ...
            <span className="font-bold text-gray-900 mt-0.5 block">{locLat?.toFixed(3)}°</span>
          </div>
          <div className="bg-gray-50 border border-gray-200 p-2.5 rounded-lg">
            <span className="text-gray-500 block">Longitude</span>
            <span className="font-bold text-gray-900 mt-0.5 block">{locLng?.toFixed(3)}°</span>
          </div>
          <div className="bg-gray-50 border border-gray-200 p-2.5 rounded-lg">
            <span className="text-gray-500 block">Time Offset</span>
            <span className="font-bold text-gray-900 mt-0.5 block">{approxOffset >= 0 ? '+' : ''}{approxOffset} hrs</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 bg-white/90 border border-gray-200 rounded-lg p-6 shadow-none space-y-6">

          <div className="flex items-center justify-between border-b border-gray-200 pb-3">
            <div className="flex items-center space-x-2">
              <Compass className="w-4 h-4 text-gray-900 animate-spin-slow" />
              <h3 className="text-sm font-sans font-bold text-amber-600 uppercase tracking-wider">Current Celestial Positions Table</h3>
            </div>
          ...
          {loading ? (
            <div className="h-44 flex items-center justify-center space-x-2.5">
              ...
              <span className="text-xs font-sans text-gray-500">Updating celestial longitude tables...</span>
            </div>
          ) : positions ? (
            <div className="space-y-6">
              ...
              <div className="overflow-x-auto border border-gray-200 rounded-lg bg-white">
                <table className="w-full border-collapse text-left text-xs text-gray-900">
                  <thead>
                    <tr className="border-b border-gray-200 bg-slate-body text-[10px] uppercase font-mono tracking-wider text-amber-600">
                  ...
                  <tbody className="divide-y divide-orange-500/5 font-sans">
                    {[
                      ...
                      <tr key={idx} className="hover:bg-orange-950/[0.04] transition-colors">
                        <td className="px-4 py-3 font-semibold text-gray-900 font-sans flex items-center space-x-2">
                        ...
                        <td className="px-4 py-3 text-amber-600 font-medium">{p.rashi}</td>
                        <td className="px-4 py-3 text-gray-900">{p.nakshatra}</td>
                        ...
                        <td className="px-4 py-3 flex items-center justify-between font-mono">
                          ...
                          <span className={`text-[9px] px-1.5 py-0.2 rounded font-sans uppercase font-bold tracking-wider ${p.retro ? 'bg-whitember-500/10 text-amber-600' : 'bg-emerald-500/10 text-[var(--color-occult-purple)]merald-500'
                            }`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans">
                ...
                <div className="bg-whitember-950/20 border border-amber-500/25 p-4 rounded-lg space-y-2">
                  <div className="flex items-center space-x-2 text-amber-600">
                    <AlertTriangle className="w-4 h-4 animate-bounce" />
                    <span className="font-bold uppercase tracking-wider">MERCURY RETROGRADE ACTIVE</span>
                  </div>
                  <p className="text-gray-500 leading-relaxed leading-normal">
                ...
                <div className="bg-emerald-950/15 border border-emerald-500/20 p-4 rounded-lg space-y-2">
                  <div className="flex items-center space-x-2 text-[var(--color-occult-purple)]merald-400">
            ...
            <div className="h-44 flex items-center justify-center text-xs text-gray-500 font-sans">
        ...
        <div className="lg:col-span-4 bg-white/80 border border-gray-200 rounded-lg p-6 shadow-none space-y-8">

          {/* 4. Current Transit Wheel Visualization */}
          <div className="space-y-3 text-center">
            <h4 className="text-xs font-sans font-bold text-amber-600 uppercase tracking-widest">360° Sidereal Transit Wheel</h4>
            {renderZodiacTransitWheel()}
            <p className="text-[10px] text-gray-500 font-sans">Planetary coordinates mapped live relative to Sidereal Zero point (0° Aries)</p>
          </div>

          {/* 13. Upcoming Major Cosmic Schedules (Events calendar) */}
          <div className="space-y-3">
            <h4 className="text-xs font-sans font-bold text-amber-600 uppercase tracking-widest border-b border-gray-200 pb-1.5">Cosmic Transit Calendar (2026)</h4>
            <div className="space-y-3 font-sans">
              {TRANSIT_EVENTS.map((ev, idx) => (
                <div key={idx} className="p-3 bg-white border border-purple-100 hover:border-gray-200 rounded-lg text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900">{ev.event}</span>
                    <span className="text-[10px] text-amber-600 font-mono font-semibold">{ev.date}</span>
                  </div>
                  <p className="text-[10.5px] text-gray-500 leading-snug">{ev.effect}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-sans font-bold text-amber-600 uppercase tracking-wider">Dynamic Celestial Transit Forecasts</h3>
          </div>

          <div className="flex bg-white border border-gray-200 rounded-lg p-0.5 text-xs font-bold">
            {[
              ...
              <button
                ...
                className={`py-1.5 px-3 rounded transition-all cursor-pointer ${activeTabTransitForecast === tab.id ? 'bg-orange-600 text-gray-900 shadow-none' : 'text-gray-500 hover:text-gray-900'
                  }`}
              >
                {tab.label}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-left font-sans leading-relaxed">

          {/* General Forecast Box based on current tab selection */}
          <div className="p-5 bg-white/90 border border-purple-100 rounded-lg space-y-2.5">
            <h4 className="font-bold text-[var(--color-occult-purple)] uppercase tracking-widest text-[11px]">General Prognosis Analysis</h4>
            {activeTabTransitForecast === 'daily' && (
              <p className="text-gray-450 text-gray-500">
          ...
          <div className="space-y-4">
            <h4 className="font-bold text-[var(--color-occult-purple)] uppercase tracking-widest text-[11px] border-b border-purple-100 pb-1 block">Sectors Ripple Impact Mapped</h4>

            <div className="grid grid-cols-2 gap-3 text-left">

              <div className="p-3 bg-white rounded-lg border border-purple-100">
                <span className="text-[10px] text-gray-500 font-bold block uppercase">💼 WORKSPACE IMPACT</span>
                <p className="text-[10.5px] text-gray-900 mt-1">Excellent for planning architecture. Practice restraint with written email exchanges to prevent Mercury retro struggles.</p>
              </div>

              <div className="p-3 bg-white rounded-lg border border-purple-100">
                <span className="text-[10px] text-gray-500 font-bold block uppercase">❤️ ROMANTIC IMPACT</span>
                <p className="text-[10.5px] text-gray-900 mt-1">Harmonious waves govern personal unions. Maintain stellar, empathetic hearing postures to dissolve family friction.</p>
              </div>

              <div className="p-3 bg-white rounded-lg border border-purple-100">
                <span className="text-[10px] text-gray-500 font-bold block uppercase">💰 FINANCIAL IMPACT</span>
                <p className="text-[10.5px] text-gray-900 mt-1">Market vectors indicate subtle volatility. Safeguard liquidity reserves and wait for Mercury to switch direct next month.</p>
              </div>

              <div className="p-3 bg-white rounded-lg border border-purple-100">
                <span className="text-[10px] text-gray-500 font-bold block uppercase">🧘 SPIRITUAL IMPACT</span>
                <p className="text-[10.5px] text-gray-900 mt-1">High-vibrational period for chants, Gayatri mantra sadhana, and charity offerings to local ashrams.</p>
              </div>
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-6 text-xs text-left space-y-4">
        <h3 className="text-sm font-sans font-bold text-amber-600 uppercase tracking-widest border-b border-purple-100 pb-2">Technical Observatory FAQ</h3>
        <div className="space-y-4">
          {FAQS.map((faq, idx) => (
      ...
      <section className="space-y-4 text-left">
        <h4 className="text-xs font-sans font-bold text-amber-600 uppercase tracking-widest">Explore Alternate Astronomical modules</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs font-sans">
          {[
            ...
            <div key={idx} className="p-4 bg-white border border-purple-100 rounded-lg space-y-1.5 hover:border-gray-200 transition-all">
              <span className="text-[9.5px] uppercase tracking-wider text-amber-600 font-mono block">{item.tag}</span>
