...
export default function CertificateView({ certificate }: CertificateViewProps) {
  ...
  return (
    <div className="bg-[#080402] border border-orange-500/20 rounded-3xl p-4 sm:p-8 max-w-4xl mx-auto shadow-2xl sacred-glow" id="certificate-viewer">
      
      {/* Certification frame with sacred luxury styling */}
      <div 
        id="printable-cert-frame"
        className="relative bg-[#0d0705] text-white border-4 border-amber-500/80 p-6 sm:p-10 rounded-xl overflow-hidden font-serif select-none"
      ...
      >
        ...
        {/* Framing border line */}
        <div className="border border-orange-500/20 p-4 sm:p-6 rounded-lg flex flex-col items-center text-center">
          ...
          <div className="flex flex-col items-center mb-4">
            ...
            <p className="text-[10px] text-orange-400 uppercase tracking-widest font-mono mt-0.5">
          ...
          {/* Student details */}
          <p className="text-xs text-orange-400 font-serif italic mb-3">This sacred credential affirms that</p>
          <p className="text-2xl sm:text-3xl font-bold tracking-wide text-white uppercase drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] my-2">
            {certificate.studentName}
          </p>

          <p className="text-xs sm:text-sm text-gray-300 max-w-lg leading-relaxed font-serif my-3 px-2">
            has successfully achieved the degree of <span className="text-amber-400 font-bold font-serif">{certificate.courseTitle}</span> after reviewing and masterfully executing the comprehensive certified curriculum, scriptural examinations (MCQs) and evaluations.
          </p>

          <div className="w-16 h-[1px] bg-amber-500/30 my-4"></div>

          {/* Verification, signatures & metadata */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full items-end mt-4 pt-4 border-t border-orange-500/10">
            
            {/* Left signature */}
            <div className="flex flex-col items-center justify-end">
              <span className="font-serif italic text-sm text-orange-400 font-bold tracking-wider">आचार्य रामचन्द्र</span>
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="w-16 h-16 bg-[#160d0a] p-1 border border-amber-500/20 rounded flex items-center justify-center shadow-inner">
                ...
                <svg className="w-full h-full text-amber-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M3 3h4v4H3zM17 3h4v4h-4zM3 17h4v4H3zM14 14h2v2h-2zM18 18h3v3h-3zM14 18h2v3h-2zM18 14h3v2h-3z" />
                </svg>
              </div>
              <p className="text-[8px] text-gray-400 mt-1.5 font-mono uppercase tracking-wider">Verify ID: SG-{certificate.id.slice(6, 11) || "10291"}</p>
            </div>

            {/* Right signature */}
            <div className="flex flex-col items-center justify-end">
              <span className="font-serif italic text-xs text-orange-300">Sankalp AI Board</span>
              <div className="w-24 h-px bg-amber-500/20 my-1"></div>
          ...
          <div className="mt-6 border-t border-orange-500/5 w-full pt-3 flex flex-col sm:flex-row items-center justify-between text-[9px] text-gray-500 font-mono">
            <p>Cryptographic Hash: <span className="font-bold text-amber-500">{certificate.id}</span></p>
      ...
      <div className="mt-5 flex flex-col sm:flex-row items-center justify-between gap-4 font-sans text-xs">
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <button
            onClick={handlePrint}
            className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-5 py-2.5 bg-[#120703] hover:bg-orange-950/20 text-orange-400 hover:text-orange-300 rounded-xl border border-orange-500/30 transition-all cursor-pointer font-serif tracking-widest font-bold text-[10px] uppercase"
          >
