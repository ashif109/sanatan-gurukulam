...
try {
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (config.firestoreDatabaseId) {
      adminDb = getFirestore(app, config.firestoreDatabaseId);
    } else {
...
async function getCollection<T>(collectionName: string, memoryFallback: T[]): Promise<T[]> {
  if (adminDb) {
    try {
      ...
      console.warn(`Firestore getCollection error for "${collectionName}":`, err, "Using local cache.");
      return memoryFallback;
    }
  }
  return memoryFallback;
...
app.post('/api/courses', async (req: Request, res: Response) => {
  try {
    const { title, description, category, difficulty, thumbnail, price, highlights, instructorId, instructorName } = req.body;
